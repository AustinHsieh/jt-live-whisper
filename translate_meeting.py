...
def _detect_input_devices_sd():
    """列舉 sounddevice 輸入裝置，並額外標記聚集裝置與 Loopback。"""
    import sounddevice as sd
    ...
    for i, dev in enumerate(devices):
        ...
        if aggregate_dev is None:
            if "聚集" in name or "aggregate" in name.lower():
                aggregate_dev = entry
            elif IS_MACOS and ch >= 3 and not _is_loopback_device(name):
                aggregate_dev = entry

        if loopback_dev is None and _is_loopback_device(name):
            loopback_dev = entry

    return input_devices, aggregate_dev, loopback_dev
...
def list_audio_devices(model_path, asr_source="playback"):
    ...
    loopback_dev = None
    try:
        _, aggregate_dev, loopback_dev = _detect_input_devices_sd()
    except Exception:
        pass
    ...
    if asr_source == "mixed":
        ...
        for dev_id, dev_name in devices:
            if _is_loopback_device(dev_name):
                ...
                return dev_id
        if loopback_dev:
            print(f"{C_WARN}[提醒] SDL2 未列出 {_LOOPBACK_LABEL}，請手動選擇對應裝置{RESET}")
        else:
...
def list_audio_devices_sd(asr_source="playback"):
    ...
    input_devices, aggregate_dev, loopback_dev = _detect_input_devices_sd()
    ...
    if asr_source == "mixed":
        ...
        if loopback_dev is not None:
            dev_id, dev_name, _, _ = loopback_dev
            print(f"  {C_OK}ASR 裝置: [{dev_id}] {dev_name}{RESET}")
            return dev_id
        print(f"{C_WARN}[提醒] 未偵測到 {_LOOPBACK_LABEL}，請手動選擇音訊裝置{RESET}")
...
def _llm_generate(prompt, model, host, port, server_type, stream=False,
                  ...
                  on_line=None, api_key="", provider="local"):
...
def run_stream(capture_id: int, translator, model_name: str, model_path: str,
    ...
    sdl_devices = _enumerate_sdl_devices(model_path)
    capture_name = next((name for dev_id, name in sdl_devices if dev_id == capture_id), f"SDL #{capture_id}")
    ...
    print(f"  {C_WHITE}音訊裝置: [{capture_id}] {capture_name}{RESET}")
    if translator:
    ...
    audio_monitor = None
    audio_probe = {"seq": 0, "rms": 0.0, "at": 0.0}
    ...
    # 用獨立 loopback probe 驗證 ASR 音源，避免被 --rec-device 誤導。
    audio_monitor = _start_audio_monitor(preferred_name=capture_name, probe=audio_probe)
    _verify_realtime_audio(capture_name, probe=audio_probe)

    listen_hints = {
        "en2zh": "播放英文音訊即可看到翻譯",
        "zh2en": "播放中文音訊即可看到英文翻譯",
        "ja2zh": "播放日文音訊即可看到中文翻譯",
        "zh2ja": "播放中文音訊即可看到日文翻譯",
        "en": "播放英文音訊即可看到字幕",
        "zh": "播放中文音訊即可看到字幕",
        "ja": "播放日文音訊即可看到字幕",
    }
...
def run_stream_moonshine(capture_id: int, translator, moonshine_model_name: str,
    ...
    if IS_WINDOWS and capture_id == WASAPI_LOOPBACK_ID:
        ...
        capture_name = f"WASAPI Loopback ({wb_info['name']})"
    else:
    ...
    print(f"  {C_WHITE}音訊裝置: {capture_name} ({sd_samplerate}Hz / {sd_channels}ch){RESET}")
    if translator:
        if isinstance(translator, OllamaTranslator):
            _srv_type_label = "Ollama" if translator.server_type == "ollama" else "OpenAI 相容"
            print(f"  {C_OK}翻譯引擎: {translator.model} @ {translator.host}:{translator.port}（{_srv_type_label}）{RESET}")
    ...
    _verify_realtime_audio(capture_name, sd_samplerate, sd_channels)
...
def run_stream_remote(capture_id: int, translator, model_name: str,
    ...
    # ── 去重 ──
    recent_subtitles = _RecentSubtitleDeduper()
        ...
        while True:
            ...
            for line in lines:
                ...
                if hallucination_check(line):
                    continue
                # 去重
                if recent_subtitles.is_duplicate(line):
                    continue
                recent_subtitles.remember(line)
                # 顯示 + 翻譯
...
def call_llm_raw(prompt, llm_cfg, timeout=300, spinner=None, live_output=False,
                 think=None, on_line=None):
    """直接呼叫指定摘要模型來源，支援本機 LLM 與 OpenAI 官方 API。"""
    return _llm_generate(
        prompt,
        llm_cfg.model,
        llm_cfg.host,
...
def query_llm_num_ctx(llm_cfg):
    """查詢摘要模型 context window；OpenAI 官方 API 沒有穩定查詢介面時回傳 None。"""
    if not llm_cfg or llm_cfg.provider == "openai":
        return None
    return query_ollama_num_ctx(
        llm_cfg.model,
        llm_cfg.host,
        llm_cfg.port,
        server_type=llm_cfg.server_type,
    )
...
def summarize_log_file(input_path, model, host, port, server_type="ollama",
                       ...
                       audio_path="", summary_rounds=1, summary_cfg=None):
...
---"""
        ...
        _tc_result = call_llm_raw(_tc_prompt, summary_cfg, spinner=sbar_tc,
                                  live_output=True)
        sbar_tc.stop()
    ...
    if summary_rounds > 1:
        ...
        for ri in range(2, min(summary_rounds, 3) + 1):
            ...
            if len(chunks) <= 1:
                ...
                _r_result = call_llm_raw(_r_prompt, summary_cfg, spinner=sbar_r,
                                         live_output=False)
            else:
                _r_segs = []
                for ci, chunk in enumerate(chunks):
                    ...
                    _r_seg = call_llm_raw(_r_prompt, summary_cfg, spinner=sbar_r,
                                          live_output=False)
                ...
                _r_result = call_llm_raw(_r_merge_prompt, summary_cfg, spinner=sbar_r,
                                         live_output=False)
            sbar_r.freeze()
            sbar_r.stop()
        ...
        summary = call_llm_raw(_merge_prompt, summary_cfg, spinner=sbar_m,
                               live_output=True)
        sbar_m.freeze()
        sbar_m.stop()
...
_live_audio_probe = {"seq": 0, "rms": 0.0, "at": 0.0}

def _update_audio_probe(probe, rms, now=None):
    """更新指定的音訊 probe 狀態。"""
    if probe is None:
        return
    if now is None:
        now = time.monotonic()
    probe["seq"] = probe.get("seq", 0) + 1
    probe["rms"] = rms
    probe["at"] = now
...
def _push_rms(rms):
    ...
    now = time.monotonic()
    _update_audio_probe(_live_audio_probe, rms, now)
    if _webui_queue is not None and now - _webui_rms_last[0] > 0.2:
        _webui_rms_last[0] = now
        _webui_send({"type": "rms", "value": round(rms, 4)})


def _verify_realtime_audio(source_name, samplerate=None, channels=None, timeout=3.0, rms_floor=1e-4, probe=None):
    """等待即時音訊流入，並輸出明確的連線/靜音警告。"""
    probe = probe or _live_audio_probe
    start_seq = probe.get("seq", 0)
    deadline = time.monotonic() + timeout
    peak_rms = 0.0
    saw_audio = False

    while time.monotonic() < deadline:
        if probe.get("seq", 0) > start_seq:
            saw_audio = True
            peak_rms = max(peak_rms, probe.get("rms", 0.0))
        time.sleep(0.25)

    if saw_audio and peak_rms >= rms_floor:
        details = []
        if source_name:
            details.append(f"裝置 {source_name}")
        if samplerate:
            details.append(f"取樣率 {samplerate}Hz")
        if channels:
            details.append(f"{channels}ch")
        details.append(f"RMS 峰值 {peak_rms:.4f}")
        print(f"  {C_DIM}音訊已連接（{'，'.join(details)}）{RESET}", flush=True)
        return True

    if saw_audio:
        print(f"  {C_HIGHLIGHT}[警告] 已收到音訊資料，但 3 秒內未偵測到明顯聲音（RMS 峰值 {peak_rms:.4f}）{RESET}", flush=True)
    else:
        print(f"  {C_HIGHLIGHT}[警告] 3 秒內未收到音訊資料{RESET}", flush=True)
    print(f"  {C_DIM}請確認系統喇叭正在播放聲音，且音訊已導到 {source_name}；必要時可用 --device 指定正確裝置{RESET}", flush=True)
    return False
...
def _start_audio_monitor(preferred_name=None, probe=None):
    ...
    bh_id = None
    preferred_norm = preferred_name.strip().casefold() if preferred_name else None
    if preferred_norm:
        for dev_id, dev_name in candidates:
            if dev_name.strip().casefold() == preferred_norm:
                bh_id = dev_id
                break
        if bh_id is None:
            for dev_id, dev_name in candidates:
                dev_norm = dev_name.strip().casefold()
                if preferred_norm in dev_norm or dev_norm in preferred_norm:
                    bh_id = dev_id
                    break
    if bh_id is None:
        bh_id = candidates[0][0]
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="即時 ASR 音源相容模式 (playback=僅播放聲音 / mixed=輸出+麥克風；mixed 等同啟用 --mic)")
    parser.add_argument(
...
def _resolve_summary_config(args, host=None, port=None, server_type=None):
    ...
    if provider == "openai":
        ...
        if not api_key:
            print(
                f"{C_HIGHLIGHT}[錯誤] --summary-provider openai 需要先在 config.json 設定 openai_api_key{RESET}",
                file=sys.stderr,
            )
            sys.exit(1)
        if not args.summary_model:
            print(
                f"{C_HIGHLIGHT}[錯誤] --summary-provider openai 時必須明確指定 --summary-model{RESET}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def main():
    ...
    if not getattr(args, "asr_source", None):
    ...
    if args.input:
        ...
        if not _has_cli_args:
            ...
            if need_translate_cli:
                ...
                engine = "llm"
            summary_model = args.summary_model or SUMMARY_DEFAULT_MODEL
            if do_summarize:
        ...
        need_llm_correct = (
            ...
            and (not summary_cfg or summary_cfg.provider == "local")
        )
        ...
        translate_srv_label = ""
        if need_llm_translate or need_llm_correct:
            if not server_type:
                server_type = _detect_llm_server(host, port)
            if server_type:
                srv_label = "Ollama" if server_type == "ollama" else "OpenAI 相容"
                translate_srv_label = srv_label
                ...
                label = "LLM" if need_llm_translate else "LLM 校正"
        ...
        if need_llm_summary and summary_cfg:
        ...
        can_llm_correct = (
            ollama_available
            and summary_mode in ("both", "correct_only")
            and (not summary_cfg or summary_cfg.provider == "local")
        )
        if need_translate:
        ...
        if summary_mode in ("both", "summary") and summary_cfg:
        ...
        # 逐檔處理
        _do_llm_correct = can_llm_correct
            ...
            for lp, orig_fpath, sess_dir in log_paths:
                ...
                try:
                    ...
                    out_path, _, html_path = summarize_log_file(
                        ...
                        summary_cfg.host,
                        summary_cfg.port,
                        server_type=summary_cfg.server_type,
                        topic=meeting_topic,
                        metadata=_meta,
    ...
    if args.summarize is not None:
        ...
        print(f"{C_DIM}{'─' * 60}{RESET}")
        print(f"  {C_DIM}摘要模型: {model} ({_summary_target_label(summary_cfg)}){RESET}")

        print(f"  {C_DIM}正在連接摘要模型來源...{RESET}", end=" ", flush=True)
        _webui_send({"type": "progress", "stage": "載入中", "detail": f"連接摘要模型來源（{_summary_target_label(summary_cfg)}）"})
        ...
        if summary_ok:
            ...
            if remote_set and model not in remote_set:
                print(f"\n{C_HIGHLIGHT}[警告] 模型 {model} 不在可用清單中，可用模型: {', '.join(sorted(remote_set))}{RESET}")
            else:
...
---"""
    ...
    if cli_mode:
        ...
        asr_source = getattr(args, "asr_source", None) or "playback"
        if args.mic and asr_source == "mixed":
            print(f"{C_WARN}[提醒] --mic 與 --asr-source mixed 語意不同；即時雙路模式將以 --mic 為準{RESET}")
            asr_source = "playback"
            args.asr_source = asr_source
        ...
        if use_remote_cli:
            ...
            if args.device is not None:
                ...
                if not _sdl_devs:
                    ...
                    capture_id = auto_select_device(_probe_path, asr_source)
