...
**整合**
- 重新補回 `--summary-provider openai`：批次摘要與離線處理自動摘要可直接使用 OpenAI 官方 API，`config.json` 支援 `openai_api_key`
- 新增 `SummaryLLMConfig` / `call_llm_raw()` / `query_llm_num_ctx()` 摘要路徑封裝，讓本機 LLM、OpenAI 相容伺服器、OpenAI 官方 API 共用同一套摘要流程
- 新增 `--asr-source` 相容參數：`mixed` 會自動對應到目前的 `--mic` 雙路即時辨識流程
- 即時字幕去重改為時間窗 dedupe，避免靜音或重複 chunk 時重送最後一句；`C_WHITE` 改回終端預設前景色，避免顏色殘留
