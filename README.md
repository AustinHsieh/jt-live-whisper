...
**共通：**
- 本地端 LLM 伺服器（推薦 [Ollama](https://ollama.com/)，翻譯/摘要用。推薦搭配 [NVIDIA DGX Spark](https://www.nvidia.com/zh-tw/products/workstations/dgx-spark/) 執行 Ollama，CP 值高。**沒有本地 LLM 伺服器也能做摘要**：可改用 OpenAI 官方 API；若完全不接 LLM，程式仍可切換為 NLLB/Argos 離線翻譯引擎，但摘要功能需要 LLM）
