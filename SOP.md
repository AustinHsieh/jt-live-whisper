...
> 若要改用 OpenAI 官方模型做摘要，可在 `--summarize` 或 `--input ... --summarize` 搭配 `--summary-provider openai --summary-model ...`。此時只有摘要請求會送到 OpenAI，其他流程仍依你的本機 / 區網設定執行。
...
| `--asr-source SRC` | 即時 ASR 音源相容模式 (`playback` / `mixed`，`mixed` 等同啟用 `--mic`) | `playback` |
...
| `--summary-model MODEL` | 摘要用的 LLM 模型 | `local: gpt-oss:120b / openai: 必填` |
...
# 改用 OpenAI 官方模型做摘要
./start.sh --summarize logs/log.txt --summary-provider openai --summary-model gpt-5-mini
```
